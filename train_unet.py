...
#  HAM10000 dataset  |  UNET_light + EfficientNetB0
# =============================================================================
...
if missing_masks:
    print(f"WARNING: {len(missing_masks)} images have no mask — {missing_masks[:5]}")
else:
    print(f"All {len(img_files)} images have matching masks.")

# =============================================================================
#  SECTION 2 — EDA visualisations  (optional, comment out to skip)
# =============================================================================

def display_image_and_mask(n=5, seed=42):
    np.random.seed(seed)
    ...
    for i in range(n):
        ...
        mask = Image.open(os.path.join(MASK_DIR, os.path.splitext(img_files[idx])[0] + "_segmentation.png"))
        axs[0, i].imshow(img);  axs[0, i].set_title("Image"); axs[0, i].axis("off")
        axs[1, i].imshow(mask); axs[1, i].set_title("Mask");  axs[1, i].axis("off")
    plt.tight_layout()
    plt.show()

def display_image_with_mask(n=5, seed=42):
    np.random.seed(seed)
    fig, axs = plt.subplots(1, n, figsize=(20, 5))
    for i in range(n):
        idx      = np.random.randint(0, len(img_files))
        img_np   = np.array(Image.open(os.path.join(IMG_DIR,  img_files[idx])))
        mask_np  = np.array(Image.open(os.path.join(MASK_DIR, os.path.splitext(img_files[idx])[0] + "_segmentation.png")))
        axs[i].imshow(img_np)
...
# =============================================================================
#  SECTION 5 — Loss / metrics  (single canonical definition)
# =============================================================================
...
def plot_history(history):
    ...
    for ax, (train_key, val_key, title) in zip(axs, metrics):
        ax.plot(history.history[train_key], label=f"Train",      linestyle="--")
...
def display_predictions(n, test_imgs, test_masks, y_pred):
    ...
    row_titles = ["Image", "Actual Mask", "Predicted Mask"]
    for i in range(n):
        dice = dice_coefficient(test_masks[i], y_pred[i]).numpy()
        axs[0, i].imshow(test_imgs[i]);  axs[0, i].set_title("Image");       axs[0, i].axis("off")
        axs[1, i].imshow(test_masks[i]); axs[1, i].set_title("Actual Mask"); axs[1, i].axis("off")
        axs[2, i].imshow(y_pred[i]);     axs[2, i].set_title(f"Pred\nDice:{dice:.3f}"); axs[2, i].axis("off")
    plt.tight_layout()
    plt.show()
...
# =============================================================================
#  SECTION 10 — TFLite export + copy to model folder
# =============================================================================
...
# Copy into the app's model folder
...
concrete_func = full_model.get_concrete_function(tf.TensorSpec((1, IMG_SIZE, IMG_SIZE, 3), tf.float32))
frozen_func   = convert_variables_to_constants_v2(concrete_func)
flops_profile = profile(graph=frozen_func.graph, options=ProfileOptionBuilder.float_operation())
