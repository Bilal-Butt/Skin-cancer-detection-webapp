...
> "Skin cancer accounts for 33.33% of all cancer cases globally. Early detection is not just helpful — it's life-saving." — World Health Organization
...
|--------|-----------|
| **Test Accuracy** | 95.70% |
| **Test Precision** | 94.28% |
| **Test Recall** | 90.86% |
| **Test Dice Coefficient** | 0.8855 |
| **Test Loss (Binary CE)** | 0.1098 |
 
Best validation Dice during training: **0.8800 at epoch 10**. Model footprint: **440K parameters · 1.32 GFLOPs · ~5.3 MB H5 / TFLite-converted variant for inference**.
 
### Classification (EfficientNetB3, 7 classes)
 
| Phase | Epochs | Best Validation Accuracy |
|-------|--------|--------------------------|
| **Phase 1** — head only (frozen base) | 8 | 77.12% |
| **Phase 2** — fine-tune last 50 layers | 20 (epochs 9–28) | 81.72% |
| **Final test accuracy** | — | **80.44%** |
...
**Why B3 and not B0 like before?** B3 has a richer feature hierarchy and a preferred input resolution of 300×300 (vs B0's 224×224). The original code was also forcing B0 into a non-native 256×256, which was a suboptimal compromise. Switching to B3 at its native size gives about 2–4% better accuracy without the compute cost of B4 or above.
...
| **Phase 2** | Last 50 base layers + head | 20 (epochs 9–28) | 3×10⁻⁵ | Cosine Decay → 0 | Focal Loss + Class Weights |
 
Phase 1 is about warming up — teaching the new head what skin lesion features matter, while keeping EfficientNetB3's pretrained weights completely untouched. Phase 2 is careful fine-tuning where the last 50 base layers slowly adapt their ImageNet representations toward dermoscopic patterns. (Including the 9 head layers, this gives 59 trainable layers in Phase 2.)
...
| **U-Net (ours)** | **95.70%** | **~30 min on T4** | **2.70 GB** |
...
👉 **[Download Models — Google Drive]([https://drive.google.com/your-link-here](https://drive.google.com/drive/folders/1FUaDRcDsSuvH1SiBMEfQvFpkwiooaYos?usp=sharing))**
...
Open source under the **MIT License**. Use it, build on it, improve it.
